...
public class LaunchCommand extends Command {
    ...
    public boolean execute(Robot target) throws JSONException {
        ...
        if (args.length < 3) {
            ...
            JSONObject response = responseHandler.createErrorResponse("Invalid launch command. Please provide robot type and name.");
        ...
        if (world.isRobotNameTaken(robotName)) {
            JSONObject response = responseHandler.createErrorResponse("Robot name is already taken");
            target.setStatus(response.toString());
            return false;
        }
        ...
        if (world.placeRobot(target)) {
            ...
            JSONObject response = responseHandler.createErrorResponse("Failed to place robot in the world");
            target.setStatus(response.toString());
            return false;
        }
    }
